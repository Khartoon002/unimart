...
export default function CartPage() {
  ...
  return (
    <>
              ...
              {items.map((item) => (
                <motion.div key={item.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, x: -40 }}
                  ...
                  <Link href={`/product/${item.productId}`} className="flex-shrink-0">
                    <div className="relative w-16 h-16 md:w-20 md:h-20 rounded-xl overflow-hidden" style={{ background: "var(--color-surface-2)" }}>
      ...
      {/* Mobile sticky checkout bar — sits above the MobileNav (56px) */}
      <div className="lg:hidden fixed bottom-14 left-0 right-0 z-40 px-4 pb-2 pt-3"
        style={{ background: "var(--color-bg)", borderTop: "1px solid var(--color-border)" }}>
        <div className="flex items-center gap-3">
          <div className="flex-1">
            <p className="text-xs" style={{ color: "var(--color-text-3)" }}>Total</p>
            <p className="font-bold text-base">{formatPrice(total)}</p>
          </div>
          <Link href="/checkout" className="flex-shrink-0">
            <button className="h-11 px-6 rounded-2xl font-semibold flex items-center gap-2 text-sm"
              style={{ background: "var(--color-primary)", color: "#fff" }}>
