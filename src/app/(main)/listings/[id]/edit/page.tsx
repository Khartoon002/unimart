...
import { Trash2, X, Plus, Wand2 } from "lucide-react";
...
export default function EditListingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Initialise variants once product loads
  useEffect(() => {
    if (product?.variants) {
      setVariants(
        product.variants.map((v: { name: string; options: { label: string }[] }) => ({
          name: v.name,
          options: v.options.map((o: { label: string }) => o.label),
        }))
      );
    }
    if (product?.tags) setTags(product.tags);
    if (product?.images?.length) setImageUrls(product.images);
  }, [product]);
  ...
  function onSubmit(data: CreateProductInput) {
    ...
    startTransition(async () => {
      const result = await updateProduct(id, { ...data, tags, images: validImages, variants: validVariants });
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="p-5 rounded-2xl space-y-4" style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)" }}>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select {...register("status")} className="w-full h-11 px-4 rounded-xl text-sm outline-none"
                style={{ background: "var(--color-surface-2)", border: "1px solid var(--color-border)", color: "var(--color-text-1)" }}>
        ...
        <div className="p-5 rounded-2xl space-y-3" style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)" }}>
          ...
          {imageUrls.length < 4 && (
            <button type="button" onClick={() => setImageUrls((p) => [...p, ""])}
              className="flex items-center gap-2 text-sm font-medium" style={{ color: "var(--color-primary)" }}>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color: "var(--color-text-2)" }}>
                Sizes, colours, or other options buyers can choose
              </p>
            </div>
            {categoryPresets.length > 0 && (
              <button type="button" onClick={applyCategoryPresets}
                className="flex-shrink-0 flex items-center gap-1.5 h-8 px-3 rounded-full text-xs font-semibold"
                style={{ background: "var(--color-primary-soft)", color: "var(--color-primary)" }}>
                <Wand2 size={12} /> Suggest for {categoryLabel}
              </button>
            )}
          </div>
