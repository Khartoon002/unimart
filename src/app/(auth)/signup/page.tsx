...
export default function SignupPage() {
  ...
  const pwStrength = !password ? 0 : password.length >= 8 && /[A-Z]/.test(password) && /[0-9]/.test(password) ? 3 : password.length >= 6 ? 2 : 1;
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input {...register("name")} placeholder="Tunde Bakare" className="w-full h-10 px-3 rounded-lg text-sm outline-none" style={field(!!errors.name)} />
              ...
              <div>
                ...
                <input {...register("matricNumber")} placeholder="19/ENG/123" className="w-full h-10 px-3 rounded-lg text-sm outline-none" style={field(!!errors.matricNumber)} />
            ...
            <div>
              ...
              <input {...register("email")} type="email" placeholder="you@university.edu.ng" className="w-full h-10 px-3 rounded-lg text-sm outline-none" style={field(!!errors.email)} />
            ...
            <div>
              ...
              <div className="relative">
                <input {...register("password")} type={showPw ? "text" : "password"} placeholder="Min. 8 chars, 1 uppercase, 1 number" className="w-full h-10 px-3 pr-10 rounded-lg text-sm outline-none" style={field(!!errors.password)} />
              ...
              {password && (
                <div className="flex gap-1 mt-1.5 items-center">
                  {[1, 2, 3].map((i) => <div key={i} className="h-1 flex-1 rounded-full transition-all" style={{ background: i <= pwStrength ? pwColors[pwStrength] : "var(--color-surface-2)" }} />)}
            ...
            <div>
              ...
              <input {...register("confirmPassword")} type="password" placeholder="••••••••" className="w-full h-10 px-3 rounded-lg text-sm outline-none" style={field(!!errors.confirmPassword)} />
            ...
            {/* Seller toggle */}
            <label className="flex items-center gap-3 p-3 rounded-xl cursor-pointer" style={{ background: wantsToSell ? "var(--color-accent-soft)" : "var(--color-surface-2)", border: `1px solid ${wantsToSell ? "var(--color-accent)" : "var(--color-border)"}` }}>
              <input {...register("wantsToSell")} type="checkbox" className="hidden" />
              <div className="w-10 h-6 rounded-full flex items-center px-0.5 flex-shrink-0 transition-colors" style={{ background: wantsToSell ? "var(--color-accent)" : "var(--color-border)" }}>
            ...
            {/* Store name (visible when wantsToSell) */}
            {wantsToSell && (
              <div>
                <label className="block text-xs font-semibold mb-1.5" style={{ color: "var(--color-text-2)" }}>Store name (optional)</label>
                <input {...register("storeName")} placeholder="e.g. Tunde Tech Hub" className="w-full h-10 px-3 rounded-lg text-sm outline-none" style={field()} />
              </div>
            )}

            <button type="submit" disabled={loading} className="w-full h-11 rounded-full font-semibold text-sm flex items-center justify-center gap-2 mt-2"
